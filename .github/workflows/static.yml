...
@media (max-width: 768px) {
    ...
    .mobile-view .goal-item {
        grid-template-columns: 1fr;
        gap: 12px;
    }
    .mobile-view .goal-actions {
        flex-direction: row;
...
<body>
  ...
  <div id="goalsView" class="app" style="display: none; flex-direction: column;">
    ...
    <main class="dashboard-container" style="flex-grow: 1; overflow-y: auto; padding: 0 10px 10px; gap: 16px;">
        ...
        <div id="activeGoalsCard" class="card glass" style="width: 100%;">
          ...
          <div id="activeGoalsList" class="goals-list">
            <div class="no-goals-message" id="noGoalsMessage">
              ...
              <button id="createFirstGoalBtn" class="ctrl-btn" style="margin-top: 10px; padding: 12px 24px; background: var(--accent); color: white; border-radius: calc(var(--border-radius) / 1.5);">Create Your First Goal</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="goalCreationCard" class="card glass" style="display: none;">
        <div class="card-header">
          <h2>Create New Goal</h2>
          <button id="cancelGoalCreationBtn" class="ctrl-btn">✕</button>
        </div>
        <form id="goalCreationForm" class="goal-form">
          <div class="form-row">
...
(function(){
    ...
    const goalCreationCard = $('goalCreationCard'), goalCreationForm = $('goalCreationForm');
    const addGoalBtn = $('addGoalBtn'), createFirstGoalBtn = $('createFirstGoalBtn');
    const cancelGoalCreationBtn = $('cancelGoalCreationBtn'), cancelGoalBtn = $('cancelGoalBtn');
        ...
        if (isMobile) {
            ...
            if (bill.isGoalPayment) {
                if (bill.canEdit && !bill.isGrayedOut) {
                    actionsHTML += `<button class="edit-goal-payment-btn" onclick="editGoalPayment(${bill.id})">✏️ Edit</button>`;
            ...
            tr.innerHTML = `
                ...
                <td class="actions-cell">
                    ...
                    ${bill.isGoalPayment && bill.canEdit && !bill.isGrayedOut ? `
                        <button class="edit-goal-payment-btn" onclick="editGoalPayment(${bill.id})">✏️ Edit</button>
        ...
        if (target.matches('.pay-btn')) {
            ...
            if (bill.isGoalPayment && bill.goalId) {
                ...
                const result = await showModal({
                    ...
                    buttons: [
                        { text: '✏️ Open Goals Tool', value: 'goals', class: 'save-btn' },
    ...
    const handleToggleDarkMode = () => { document.documentElement.classList.toggle('dark'); const isDark = document.documentElement.classList.contains('dark'); darkToggle.innerHTML = isDark ? '☀️' : '🌙'; applyMaterialStyle(); applyGlassShadow(); saveData(); };
    ...
    function createGoalElement(goal) {
        ...
        goalDiv.innerHTML = `
            <div class="goal-info">
                ...
                ${goal.notes ? `<div style="font-size: 12px; color: var(--muted);">${goal.notes}</div>` : ''}
            </div>
            <div class="goal-actions">
                <div class="goal-amount">${formatCurrency(goal.savedAmount || 0)} / ${formatCurrency(goal.amount)}</div>
                <div style="display: flex; gap: 8px;">
                    <button class="ctrl-btn" onclick="editGoal(${goal.id})" title="Modify Goal">Modify</button>
                    <button class="ctrl-btn" onclick="addGoalPayment(${goal.id})" title="Add Payment" style="background: var(--accent); color: white;">+</button>
                    <button class="ctrl-btn" onclick="deleteGoal(${goal.id})" title="Delete Goal" style="background: var(--danger); color: white;">🗑️</button>
                </div>
            </div>
        `;
        return goalDiv;
    ...
    function checkGoalFeasibility() {
        ...
        if (!nextPaydayValue) {
            showFeasibilityFeedback('warning', '⚠️ Please set your next payday in the Finance section first.');
            paymentSchedulePreview.style.display = 'none';
            previewGoalBtn.style.display = 'none';
            createGoalBtn.style.display = 'none';
            return;
        ...
        if (requiredPerPaycheck <= discretionary * 0.8) {
            ...
            showFeasibilityFeedback('warning', `⚠️ Goal is tight but possible. You'd need to save ${formatCurrency(requiredPerPaycheck)} per paycheck.`);
            previewGoalBtn.style.display = 'inline-block';
        } else {
    ...
    function recalculateGoalPayments(goalId) {
        ...
        if (unlockedBills.length === 0) {
            ...
            if (Math.abs(lockedTotal - goalRemaining) > 0.01) {
                console.warn(`⚠️ Warning: Locked total (${formatCurrency(lockedTotal)}) doesn't match goal remaining (${formatCurrency(goalRemaining)})`);
            }
            return;
        }
        ...
        if (remainingToAllocate > 0) {
            ...
            console.log('⚠️ No remaining amount to allocate to unlocked payments');
        }
    }
    ...
    window.deleteGoalPayment = function(billId, buttonElement) {
        console.log(`🗑️ Delete payment called for bill ${billId}`);
        ...
        bills = bills.filter(b => b.id !== billId);
        console.log(`🗑️ Payment ${billId} deleted from bills array`);
                                ...
                                <div class="payment-controls">
                                    ...
                                    ${bill.canDelete ? `
                                        <button class="delete-payment-btn" data-bill-id="${bill.id}"
                                                onclick="deleteGoalPayment(${bill.id}, this)">🗑️ Delete</button>
                                    ` : ''}
                                </div>
                            </div>
                        `;
    ...
    [addGoalBtn, createFirstGoalBtn].forEach(btn => {
        btn.addEventListener('click', () => {
            goalCreationCard.style.display = 'block';
    ...
    [cancelGoalCreationBtn, cancelGoalBtn].forEach(btn => {
        btn.addEventListener('click', () => {
            console.log('Goal creation cancelled');
            goalCreationCard.style.display = 'none';
